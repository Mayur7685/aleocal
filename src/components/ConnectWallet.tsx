// ConnectWallet component for Aleo AleoCal
// Supports wallet connection (Leo, Puzzle, Fox, Soter) or local account generation
...
export const ConnectWallet: React.FC<ConnectWalletProps> = ({ nextPage }) => {
  ...
  // Sync wallet connection with app state
  useEffect(() => {
    if (connected && publicKey) {
      // Wallet is connected - generate a temporary local keypair for ZK computations
      // while using the wallet's address as the display identity
      (async () => {
        try {
          // Generate a temp account for local ZK proof computations
          const tempAccount = await getOrCreateAccount();

          // Use wallet address as identity but temp keypair for ZK proofs
          setAleoAccount({
            privateKey: tempAccount.privateKey, // Use temp key for local ZK computations
            viewKey: tempAccount.viewKey,
            address: publicKey, // Use wallet address as identity
          });
          ...
          toast({
            ...
            description: `${wallet?.adapter.name}: ${addressTruncate(publicKey)}`,
            status: "success",
            duration: 3000,
            isClosable: true,
          });
          ...
          toast({
            title: "Error",
            description: "Failed to initialize ZK computation keys",
            status: "error",
            duration: 5000,
            isClosable: true,
          });
          ...
          <VStack spacing={6} maxW="500px">
            ...
            <VStack spacing={3} w="100%">
              ...
              <Text fontSize="sm" color="aleocal.coal" opacity={0.6} textAlign="center">
                Connect using Leo Wallet browser extension
              </Text>
              <WalletMultiButton />
              ...
              {isLeoWalletInstalled && !connected && (
                <Text fontSize="xs" color="aleocal.pomelo" textAlign="center">
                  Make sure Leo Wallet is set to <strong>Testnet</strong> network
                </Text>
              )}
              {connecting && (
